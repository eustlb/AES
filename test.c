...
static const uint8_t gf_by3 [256] = {
...
void print2D(uint8_t to_print[STATE_ROW_SIZE][STATE_COL_SIZE]){
   for(int i = 0; i < STATE_ROW_SIZE; i++) {
      for(int j = 0; j < STATE_COL_SIZE; j++) {
         printf("%x ", to_print[i][j]);
      }
      printf("\n");
   } 
}
...
void rotate(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE], int i){
   uint8_t val;

  // Rotate i-row 1 columns to left  
  val = state[i][0];
  state[i][0] = state[i][1];
  state[i][1] = state[i][2];
  state[i][2] = state[i][3];
...
void ShiftRows(uint8_t state[STATE_ROW_SIZE][STATE_COL_SIZE]){
   ...
   // Rotate 1-row 1 columns to left
   rotate(state,1);

   // Rotate 2-row 2 columns to left
   rotate(state,2);
   rotate(state,2);

   // Rotate 3-row 3 columns to left
   rotate(state,3);
   rotate(state,3);
   rotate(state,3);
};

void MixColumns(uint8_t state [STATE_ROW_SIZE][STATE_COL_SIZE], uint8_t result [STATE_ROW_SIZE][STATE_COL_SIZE]){
   for (int j=0; j<STATE_ROW_SIZE; j++){
      result[0][j] = gf_by2[(int)state[0][j]] ^ gf_by3[(int)state[1][j]] ^ state[2][j] ^ state[3][j];
      result[1][j] = state[0][j] ^ gf_by2[(int)state[1][j]] ^ gf_by3[(int)state[2][j]] ^ state[3][j];
      result[2][j] = state[0][j] ^ state[1][j] ^ gf_by2[(int)state[2][j]] ^ gf_by3[(int)state[3][j]];
      result[3][j] = gf_by3[(int)state[0][j]] ^ state[1][j] ^ state[2][j] ^ gf_by2[(int)state[3][j]];
   }
};
...
void AESEncrypt_128block(uint8_t ciphered_block[STATE_ROW_SIZE][STATE_COL_SIZE], uint8_t block[STATE_ROW_SIZE][STATE_COL_SIZE] , uint8_t master_key[STATE_ROW_SIZE][STATE_COL_SIZE]){
   ...
   for (int k=0; k<10; k++){
      ...
      if (k != 9) {
         uint8_t temp[STATE_ROW_SIZE][STATE_COL_SIZE];
         MixColumns(ciphered_block, temp);
         copyArray2D(temp, ciphered_block,4);}
      // AddRoundKey
...
int main() {
   ...
   // // r = gf_by2[(int)test[0][0]] ^ gf_by3[(int)test[1][0]] ^ test[2][0] ^ test[3][0];
   // // printf("%x",r);
   ...
   // uint8_t roundkey_1 [STATE_ROW_SIZE][STATE_COL_SIZE];
   // copyArray2D(roundkeys[0], roundkey_1, 4);
